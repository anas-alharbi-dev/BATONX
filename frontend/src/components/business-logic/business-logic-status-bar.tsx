...
export function BusinessLogicStatusBar({
  approved,
  approvedAt,
  updatedAt,
  amending,
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {approved ? (
            <Button variant="outline" size="sm" onClick={onToggleAmend}>
              {amending ? dict.softwareShared.doneEditing : dict.businessLogic.editArtifact}
            </Button>
          ) : (
            <Button size="sm" loading={approving} onClick={onApprove}>
              {dict.businessLogic.approveArtifact}
            </Button>
          )}
        </div>
      </div>

      {approved && amending && <ImpactNotice artifact="business_logic" />}

      {stale && (
        <div className="mt-2">
          <StaleNotice artifact={dict.businessLogic.staleArtifactName} />
        </div>
      )}
