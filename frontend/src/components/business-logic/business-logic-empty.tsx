...
export function BusinessLogicEmpty({
  variant,
  slug,
  onGenerate,
  loading,
  ...
  if (variant === "needs-blueprint") {
    return (
      <EmptyState
        title={dict.businessLogic.needsBlueprintTitle}
        description={dict.businessLogic.needsBlueprintBody}
      >
        <Link
          href={`/projects/${slug}/blueprint`}
          className={buttonClasses("solid", "md")}
        >
          {dict.businessLogic.goToBlueprint}
        </Link>
      </EmptyState>
    );
  }
  ...
  return (
    <div className="mx-auto grid max-w-md justify-items-center gap-4 py-16 text-center">
      <h2 className="text-lg font-semibold text-fg">{dict.businessLogic.generateTitle}</h2>
      <p className="text-sm leading-relaxed text-fg-muted">
        {dict.businessLogic.generateBody}
      </p>

      <Button size="lg" onClick={onGenerate} loading={loading}>
        {loading ? dict.common.generatingEllipsis : dict.businessLogic.generateCta}
      </Button>
