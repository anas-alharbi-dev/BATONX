        ...
        <div className="flex flex-wrap gap-2">
          ...
          {approved ? (
            <Button variant="outline" size="sm" onClick={onToggleAmend}>
              {amending ? dict.softwareShared.doneEditing : dict.architecture.editArtifact}
            </Button>
          ) : (
            <Button size="sm" loading={approving} onClick={onApprove}>
              {dict.architecture.approveArtifact}
            </Button>
          )}
        </div>
      </div>

      {approved && amending && <ImpactNotice artifact="architecture" />}

      {stale && (
        <div className="mt-2">
          <StaleNotice artifact={dict.architecture.staleArtifactName} />
        </div>
      )}
